...
def reshape(array, ConstPixelSpacing, a, c, width, depth):
    """ array to a 220 by 220 by 160 array with uniform spacing """
    reshaped_array1 = np.zeros((a,width,depth))
    
    xp = np.linspace(0, ConstPixelSpacing[0]*(width-1), width) #this returns             #(0,CPS,2CPS,3CPS....)
    #xp = np.linspace(0, ConstPixelSpacing[0]*width, width) #not (0, CPS, 2CPS..
